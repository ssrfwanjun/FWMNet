...
class UNetConvBlock(nn.Module):
    def __init__(self, in_size, out_size, downsample):
        ...
        self.body = [FWB(n_feat=in_size, o_feat=in_size, kernel_size=3, reduction=16, bias=False, act=nn.PReLU())]# for _ in range(wab)]
...
class SALayer(nn.Module):
    def __init__(self, kernel_size=15, bias=False):
...
class FWB(nn.Module):
    ...
    def forward(self, x):
        ...
        x_dwt = self.dwt(identity_path)
        res = self.body(x_dwt)
        branch_sa = self.WSA(res)
        branch_ca = self.WCA(res)
        res = torch.cat([branch_sa, branch_ca], dim=1)
        res = self.conv1x1(res) + x_dwt
        identity = self.iwt(res)

        out = torch.cat([wavelet_path, identity], dim=1)
...
##########################################################################
##---------- FWMNet-LOL ----------
class FWMNet(nn.Module):
    def __init__(self, in_chn=3, wf=64, depth=4):
        super(FWMNet, self).__init__()
        self.depth = depth
...
if __name__ == "__main__":
    ...
    model = FWMNet(in_chn=3, wf=96, depth=4).cuda()
    out = model(input)
